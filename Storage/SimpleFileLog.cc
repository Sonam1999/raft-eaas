...
namespace LogCabin
{
    namespace Storage
    {
        ...
        SimpleFileLog::SimpleFileLog(const FilesystemUtil::File &parentDir)
            : memoryLog(), metadata(), dir(FilesystemUtil::openDir(parentDir, "SimpleFile")), lostAndFound(FilesystemUtil::openDir(dir, "SimpleFile-unknown")), currentSync(new Sync(0))
        {
